...
class DocumentMetadataViewModel: ObservableObject {
    ...
    func saveDocument(document: UIImage, title: String, mimeType: String) {
        self.storage.saveDocument(document: document, title: title, mimeType: mimeType, completion: { result in
            switch result {
            case .success():
